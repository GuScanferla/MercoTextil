...
const EspulasPanel = ({ espulas, user, onEspulaUpdate }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {showAllOrders && (
        <div className="grid gap-6">
          <h3 className="text-2xl font-bold text-white">Histórico Completo do Sistema</h3>
          <p className="text-gray-400">Todos os pedidos, manutenções e espulas criados no sistema</p>
          
          {allSystemOrders.map((order) => (
            <Card key={`${order.type}-${order.id}`} className="card-merco-large">
              <CardContent className="p-4">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center space-x-4">
                    <Badge 
                      className={`
                        ${order.type === 'order' ? 'bg-blue-600' : 
                          order.type === 'maintenance' ? 'bg-orange-600' : 'bg-green-600'} 
                  ...
                  <div className="text-right">
                    <p className="text-gray-400 text-sm">
                      Máquina: <span className="text-white">{order.machine}</span>
                    </p>
                    <p className="text-gray-400 text-sm">
                      Status: <span className="text-white">{order.status_display}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    <span className="font-medium text-gray-400">Criado:</span>
                    <p className="text-white">{formatDateTimeBrazil(order.created_at || order.updated_at)}</p>
                  </div>
                  {order.type === 'order' && (
                    <>
                      <div>
                  ...
                  {order.type === 'maintenance' && (
                    <>
                      <div>
                        <span className="font-medium text-gray-400">Motivo:</span>
                        <p className="text-white">{order.reason || '-'}</p>
                      </div>
                      <div>
                        <span className="font-medium text-gray-400">Criado por:</span>
                        <p className="text-white">{order.created_by || '-'}</p>
                  ...
                  {order.type === 'espula' && (
                    <>
                      <div>
                        ...
                        <p className="text-white">{order.quantidade_metros || '-'}</p>
                      </div>
                      <div>
                        <span className="font-medium text-gray-400">Carga:</span>
                        <p className="text-white">{order.carga || '-'}</p>
                      </div>
                    </>
                  )}
                  <div>
                    <span className="font-medium text-gray-400">Layout:</span>
                    <p className="text-white">{order.layout_type || 'N/A'}</p>
                  </div>
                </div>

                {(order.observacao || order.observacoes) && (
                  <div className="mt-3 p-3 bg-gray-800/50 rounded">
                    <span className="font-medium text-gray-400">Observações:</span>
                    <p className="text-white text-sm">{order.observacao || order.observacoes}</p>
                  </div>
                )}
              </CardContent>
            </Card>
          ...
          {allSystemOrders.length === 0 && (
            <Card className="card-merco-large">
              <CardContent className="p-6 text-center">
                <p className="text-gray-400 text-lg">Nenhum registro encontrado no sistema.</p>
              </CardContent>
            </Card>
          )}
        </div>
